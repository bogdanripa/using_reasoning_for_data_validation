...
def generate_and_write_data():
    ...
    with open('/tmp/medicalData.csv', 'a', newline='') as csvfile:
...
def validate_data2():
    ...
    input_data = []
    with open('/tmp/medicalData.csv', 'r') as file:
...
def handler(event):
    ...
    html = display_formatted_dataframe(pd.DataFrame(get_validation_results()))
